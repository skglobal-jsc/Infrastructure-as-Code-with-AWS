...
String crop2Char(String s) {
  ...
  if (ps.length > 1) {
    ...
    charFn = s.trim().substring(0, min(s.length, 2));
  }
  return charFn.toUpperCase();
}
...
Future<File> cropCenterResizeImage(File file, int width) async{
  ...
  if (properties.width > properties.height) {
    length = properties.height;
    y = 0;
    x = (properties.width - properties.height) ~/ 2;
  }
  else {
    length = properties.width;
    x = 0;
    y = (properties.height - properties.width) ~/ 2;
  }

  File croppedFile = await FlutterNativeImage.cropImage(file.path, x, y, length, length);
  File compressedFile = await FlutterNativeImage.compressImage(croppedFile.path, quality: 100, targetWidth: width, targetHeight: width);
